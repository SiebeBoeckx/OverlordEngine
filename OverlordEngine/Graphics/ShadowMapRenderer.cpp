...
void ShadowMapRenderer::Initialize()
{
	...
	const int staticId = int(ShadowGeneratorType::Static);
	const int skinnedId = int(ShadowGeneratorType::Skinned);
	const auto staticTech = m_pShadowMapGenerator->GetTechniqueContext(staticId);
	const auto skinnedTech = m_pShadowMapGenerator->GetTechniqueContext(skinnedId);
...
void ShadowMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	const auto context = m_GeneratorTechniqueContexts[int(type)];
...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	const auto direction = XMLoadFloat4(&light.direction);
	const auto focusPosition = eyePosition + direction;
	const auto viewMatrix = XMMatrixLookAtLH(eyePosition, focusPosition, direction);
