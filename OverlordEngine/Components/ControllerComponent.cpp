...
void ControllerComponent::Initialize(const SceneContext& /*sceneContext*/)
{
	if(!m_IsInitialized)
	{
		const auto position = m_pGameObject->GetComponent<TransformComponent>()->GetPosition();
		m_ControllerDesc.position.x = position.x;
		m_ControllerDesc.position.y = position.y;
		m_ControllerDesc.position.z = position.z;
		m_ControllerDesc.userData = this;

		const auto controllerManager = m_pScene->GetPhysxProxy();
