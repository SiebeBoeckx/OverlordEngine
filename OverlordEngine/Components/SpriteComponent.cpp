...
void SpriteComponent::Draw(const SceneContext& sceneContext)
{
	...
	auto rot = MathHelper::QuaternionToEuler(pTrans->GetRotation());
	const XMFLOAT2 scale{ pTrans->GetWorldScale().x , pTrans->GetWorldScale().y };

	SpriteRenderer::Get()->AppendSprite(m_pTexture, pos, m_Color, m_Pivot, scale, rot.z, pTrans->GetPosition().z);
