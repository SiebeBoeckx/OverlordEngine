...
GameObject* CameraComponent::Pick(CollisionGroup ignoreGroups) const
{
	...
	const auto& sceneContext = GetScene()->GetSceneContext();

	// ndc
	const POINT mouse = sceneContext.pInput->GetMousePosition();
	...
	const float x = (mouse.x - halfWidth) / halfWidth; //In ndc
	const float y = -((mouse.y - halfHeight) / halfHeight); //In ndc

	// near far point
	const auto inverseVP = XMLoadFloat4x4(&GetViewProjectionInverse());
	const auto nearPoint = XMVector3TransformCoord({ x, y, 0.f, 0.f }, inverseVP);
	const auto farPoint = XMVector3TransformCoord({ x,y,1.f, 0.f }, inverseVP);
	...
	const PxVec3 rayStart = PhysxHelper::ToPxVec3(nearP);
	...
	if (physxProxy->Raycast(rayStart, dirVec, PX_MAX_F32, hit, PxHitFlag::eDEFAULT, filterData))
	{
		//Get first hit
		const auto& component = static_cast<BaseComponent*>(hit.getAnyHit(static_cast<PxU32>(0)).actor->userData);
